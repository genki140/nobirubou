...
HRESULT CD3DApplication::Create( HINSTANCE hInstance )
{
    ...
    if( m_hWnd == NULL)
    {
        ...
        WNDCLASSEX wndClass = { sizeof(WNDCLASSEX),CS_HREDRAW|CS_VREDRAW, WndProc, 0, 0, hInstance,
                              ...
                              NULL, _T("GenkiProject"),LoadIcon( hInstance, MAKEINTRESOURCE(IDI_MAIN_ICON) ) };
        ...
        m_hWnd = CreateWindow( _T("GenkiProject"), m_strWindowTitle, m_dwWindowStyle,
...
HRESULT CD3DApplication::Render3DEnvironment()
{
    ...
    if( m_bFrameMoving || m_bSingleStep )
    {
		...
		//最初はフルスクリーン
		static bool FULL_Ikkoma=false;
		if(FULL_Ikkoma == false)
			if (m_Fullscreen==true){ToggleFullscreen();DXUtil_Timer( TIMER_RESET );}
			else{DXUtil_Timer( TIMER_RESET );}
		FULL_Ikkoma=true;



		if(m_FrameLateEnable==true)
			while(1.0f/(float)m_FrameLate*(float)m_Frame>=DXUtil_Timer( TIMER_GETAPPTIME )){}
	...
	if((1.0f/(float)m_FrameLate*(float)m_Frame)<DXUtil_Timer( TIMER_GETAPPTIME ) && m_FrameLateEnable==true)
	{
		Rendered=false;
	}
	else
